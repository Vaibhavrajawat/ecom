          ...
          <div className="flex flex-col space-y-4">
            ...
            {status === "authenticated" && session?.user ? (
              <>
                <Link
                  href="/cart"
                  className="flex items-center text-base font-medium transition-colors hover:text-foreground/80 text-foreground/60 relative"
                  onClick={() => setShowMobileMenu(false)}
                >
                  <motion.div
                    key={itemCount}
                    ...
                    transition={{ duration: 0.5 }}
                  >
                    <ShoppingCart className="mr-2 h-4 w-4 cart-icon" />
                  </motion.div>
                  <AnimatePresence>
                    {itemCount > 0 && (
                      <motion.span
                        ...
                        exit={{ scale: 0, opacity: 0 }}
                        key={itemCount}
                        className="absolute top-0 left-4 h-4 w-4 rounded-full bg-gradient-to-r from-purple-600 to-blue-600 text-[10px] font-bold text-white flex items-center justify-center"
                      >
                        {itemCount}
                      </motion.span>
                    )}
